...
| 항목 | 이전 | 업데이트 |
|------|------|----------|
| 비즈니스 모델 | "완전 무료 플랫폼" | **토큰 절감가치의 15% 수익화 + 크리에이터 수익 토크나이즈** |
| 가치 제안 | "런타임 토큰 70% 절감" | **런타임 + 개발 인건비 + 설계 시간 통합 절감** |
| 생태계 비전 | Mana Points 리워드 | **룬별 수익 자동 토크나이즈 → 크리에이터 85% 분배** |
...
Creator Program — 수익의 85% 크리에이터 분배
```
...
- 절감액의 **15%만** RuneHub가 가져감
- 나머지 **85%는 크리에이터**에게 자동 토크나이즈 분배
- 사용자는 항상 순이익 → 저항 없이 채택

**왜 이 모델이 강력한가:**
- 청구 근거가 명확 ("당신이 절감한 돈의 15%만")
